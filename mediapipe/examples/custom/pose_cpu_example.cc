#include "mediapipe/examples/custom/run_graph_cpu_only.h"
...
int main() {
    ...
    try {
        PoseGraphRunner pose;
        std::string graph_config = "/src/mediapipe/graphs/pose_tracking/pose_tracking_cpu.pbtxt";

        std::cout << "Initializing pose graph (CPU version)..." << std::endl;
        if (!pose.initGraph(graph_config)) {
            std::cerr << "Failed to initialize the pose graph." << std::endl;
            return -1;
        }
        std::cout << "Pose graph initialized successfully." << std::endl;

        cv::Mat camera_frame = cv::imread("/src/mediapipe/examples/custom/far_face.jpg");
        ...
        for (int i = 0; i < warmup_iterations; i++) {
            ...
            if (!pose.processFrame(camera_frame, current_timestamp_us, landmarks)) {
                std::cerr << "Failed to process frame during warmup iteration " << i << std::endl;
                return -1;
            }
            if (i == 0) {
                std::cout << "Warmup: Found " << landmarks.size() << " poses in first frame." << std::endl;
            }
        ...
        if (!pose.processFrame(camera_frame, current_timestamp_us, final_landmarks)) {
            std::cerr << "Failed to process final verification frame." << std::endl;
            return -1;
        }

        std::cout << "Found " << final_landmarks.size() << " poses in final frame." << std::endl;

        // Pose has 33 landmarks per detected person
        for (size_t i = 0; i < final_landmarks.size(); i++) {
            std::cout << "Pose " << i << " has " << final_landmarks[i].landmarks.size() << " landmarks" << std::endl;
