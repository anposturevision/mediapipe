// Example usage of the hand landmark extraction (CPU-only version)
...
int main() {
    try {
        ...
        std::string graph_config = "/mediapipe/mediapipe/graphs/hand_tracking/hand_tracking_cpu_lib.pbtxt";

        std::cout << "Initializing hand tracking graph (CPU version)..." << std::endl;
        ...
        for (int i = 0; i < warmup_iterations; i++) {
            ...
            if (i == 0) {
                std::cout << "Warmup: Found " << landmarks.size() << " hands in first frame." << std::endl;
            }
        ...
        std::cout << "Found " << final_landmarks.size() << " hands in final frame." << std::endl;

        // Print some landmark information from final frame
        for (size_t i = 0; i < final_landmarks.size(); i++) {
            std::cout << "Hand " << i << " has " << final_landmarks[i].landmarks.size() << " landmarks" << std::endl;
