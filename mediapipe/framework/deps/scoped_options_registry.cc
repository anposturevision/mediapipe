...
RegistrationToken ScopedOptionsRegistry::Register(const mediapipe::FieldData& file_descriptor_set) {
...
const tool::Descriptor* ScopedOptionsRegistry::GetProtobufDescriptor(const std::string& type_name) {
  // Delegate to the global registry for compatibility
  return tool::OptionsRegistry::GetProtobufDescriptor(type_name);
}
...
void ScopedOptionsRegistry::FindAllExtensions(
  ...
  // Delegate to the global registry for compatibility  
  tool::OptionsRegistry::FindAllExtensions(extendee, result);
}
...
RegistrationToken EnhancedOptionsRegistry::RegisterWithRefCounting(
    const mediapipe::FieldData& file_descriptor_set,
  ...
  auto token = tool::OptionsRegistry::Register(file_descriptor_set);
  
  // Use a shared pointer to make the lambda copy-constructible
  auto token_ptr = std::make_shared<RegistrationToken>(std::move(token));
  
  return RegistrationToken([token_ptr]() {
    // The token will automatically clean up when destroyed
    // The shared_ptr keeps the token alive
  });
}
